...
*   **⚔️ The Defender (LVR Protection):** *(Trigger: HTTP/Indexer)* An event-driven risk classifier. It listens to Uniswap V4 swap events in real-time, fetches chainlink oracle volatility data, and queries **Gemini AI**. If toxic arbitrage is detected, it instantly triggers the LVRHook to dynamically raise swap fees.
